...
class AgentConfigModel(BaseModel):
    ...
    allow_dynamic_slots: bool | None = Field(
        None,
        description=(
            "Opt-in: allow the runtime to issue dynamic slots (missing inputs "
            "the platform computes at execution time, rather than only the "
            "statically pre-enumerated required/optional slots). Default OFF — "
            "when unset the key is omitted (byte-identical to prior blueprints); "
            "set True to opt in. Mirrors the `system_prompt` None-optional "
            "emission convention so default-off workflows serialize unchanged."
        ),
    )
