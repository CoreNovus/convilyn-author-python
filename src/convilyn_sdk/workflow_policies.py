...
:mod:`convilyn_sdk.policies` (``RetryPolicy``, ``TimeoutPolicy``,
...
models (canonical home: :mod:`convilyn_sdk._internal.legacy_policies`)
plus the bounded ``Literal`` vocabularies. It is intentionally **not**
part of ``convilyn_sdk.__all__`` — it carries no stability guarantee —
but stays importable as ``convilyn_sdk.workflow_policies`` for two
reasons:
...
* it keeps the granular models off the ``_internal`` import path so
  :mod:`convilyn_sdk.workflow` never reaches into ``_internal``
