...
- **Pre-releases** use PEP 440 suffixes (`1.1.1b4`). `pip install convilyn-author`
  installs the latest *stable*; `pip install --pre convilyn-author` opts into betas.
...
3. **OIDC publish (no secrets).** The tag triggers `.github/workflows/sdk-publish.yml`
   ...
   - One-time setup on PyPI: add a *Trusted Publisher* for the `convilyn-author` project
