...
`convilyn-author` (the **author** SDK, import package `convilyn_sdk`) follows
...
1. **Everything reachable as `from convilyn_sdk import X`** — i.e. every name in
   `convilyn_sdk.__all__`. That is the server + workflow builders (`ToolServer`,
   ...
   `ComplianceResult`, …).
2. **The public testing helpers** under `convilyn_sdk.testing`
...
`convilyn_sdk.__all__`, the core abstractions' contract methods, and the CLI
command tree, and asserts that nothing from `convilyn_sdk._internal` leaks into
...
- **`convilyn_sdk._internal.*`** — HMAC signature verification, the JSON-RPC
...
cohort (the `Specialist` alias + `convilyn_sdk.specialist` path, the
...
from `convilyn_sdk.workflow_policies` (the advanced, non-semver surface the
granular `with_task_policy` / `with_routing` / `with_qa_policy` builders
compose). Only their top-level `from convilyn_sdk import *Config` re-export was
dropped; prefer the five high-level knobs in `convilyn_sdk.policies`.

## Behaviour changes

| Version | Change |
|---|---|
| **2.0.0** | Removed the `1.x` deprecation cohort: `Specialist` (top-level alias) and the `convilyn_sdk.specialist` import path → `AgentRole`; `SpecialistConfigModel` / `MultiAgentConfig` aliases → `RoleConfig` / `MultiRoleConfig`; `WorkflowSpec.with_multi_agent` / `with_checkpoint` → `with_multi_role` / `with_resume_boundary`; the top-level re-export of the 13 granular `*Config` models (still importable from `convilyn_sdk.workflow_policies`). The legacy `convilyn` CLI alias was also removed (use `convilyn-author`). |
